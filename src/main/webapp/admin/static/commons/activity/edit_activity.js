...
layui.use(['form', 'layer','upload', 'laydate','croppers'], function() {
	...
	var editor = new Simditor({
		...
		upload: {
			url: getRealPath() + '/admin/activity/upload/content/img', //文件上传的接口地址
	...
	form.on("submit(editNews)", function(data) {
		...
		data.field.activeContent = editor.getValue();
		$.ajax({
			type: "POST",
			url: getRealPath() + "/admin/activity/update",
			data: data.field,
	...
	upload.render({
		elem: '#activity_img'
		, url: getRealPath() + "/admin/activity/upload/img" //必填项
		...
		,done: function(result){
			...
			$("#activityImgVal").val(result.data.src);
			$("#activity_img_view")[0].src = result.data.src
		}
		,error: function(){
			layer.msg("上传失败");
		}
