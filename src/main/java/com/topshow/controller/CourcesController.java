...
public class CourcesController {
    ...
    public Result selectAllClass(){
        List<Cources> clas =courcesService.selectAllClass();
        if (clas!=null)
             return new Result(200,"查询成功",0,clas);
        else
             return new Result(-1,"查询失败",0,clas);
    }

    //修改课表信息
    @RequestMapping(value = "/update",method = RequestMethod.POST)
    public Result updateClass(Cources cources){
        int x =courcesService.updateClass(cources);
        if (x>0)
    ...
    public Result deleteDance(int id){
        int x = courcesService.deleteClass(id);
        if (x>0) {
    ...
    public Object InsertDance(String className,String classPhoto){
        int x = courcesService.insertClass(className,new Date(),classPhoto);
        if (x>0)
